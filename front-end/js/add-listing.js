...
function geodir_save_post() {
    ...
    $button_text = jQuery('.geodir_button.save-btn').html();
    jQuery('.geodir_button.save-btn').html('<i class="fas fa-circle-notch fa-spin"></i> ' + $button_text).addClass('gd-disabled').prop('disabled', true);
    jQuery.ajax({
        type: "POST",
        url: geodir_params.ajax_url,
        ...
        success: function(data) {
            if (data.success) {
                ...
                jQuery('.geodir_button.save-btn').html($button_text).removeClass('gd-disabled').prop('disabled', false);
